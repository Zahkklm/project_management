...
def check_project_access(
    ...
    if required_role is not None and required_role == "owner" and access.role != "owner":
        raise HTTPException(
...
def create_project(
    ...
    new_project = Project(
        name=project_data.name, description=project_data.description, owner_id=current_user.id
    )
    db.add(new_project)
    db.commit()
    db.refresh(new_project)

    project_access = ProjectAccess(project_id=new_project.id, user_id=current_user.id, role="owner")
...
def get_projects(current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    accesses = db.query(ProjectAccess).filter(ProjectAccess.user_id == current_user.id).all()
...
def invite_user_to_project(
    ...
    existing_access = (
        db.query(ProjectAccess)
        .filter(ProjectAccess.project_id == project_id, ProjectAccess.user_id == invited_user.id)
        .first()
    )
    ...
    def share_project_via_email(
        ...
        return {"message": f"Invite link sent to {with_email}", "join_link": join_link}
